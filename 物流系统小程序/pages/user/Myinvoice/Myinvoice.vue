<template>
	<view class="content">
		...
		<view v-for="(item,index) in list" :key="index" style="margin:20rpx;border-radius:20rpx;background-color: #FFF;padding:24rpx;">
			<view class="flexJusp" style="align-items: center;">
				<view style="font-weight: bold;color: #333333;">{{item.company_letterhead}}</view>
				<view style="font-size: 26rpx;color: #28BC8A;" v-if="item.status==1">已完成</view>
				<view style="font-size: 26rpx;color: #F29100;" v-else>已提交</view>
			</view>
			<view class="flexJusp" style="align-items: center;margin-top:12rpx;">
				<view style="font-size: 24rpx;color: #888888;">{{item.createtime}}</view>
				<image @click="xiazai(item)" style="width: 32rpx;height: 32rpx;" v-if="item.status==1" src="/static/images/xiazai.png"></image>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			loads() {
				...
				this.$httpapi('Placeorder/UserInvoice', 'POST', {}).then(res => {
					this.list = res.data
				})
			}
		}
