		...
		<view v-for="(item,index) in list" :key="index" style="background: #FFFFFF;border-radius: 24rpx;padding:24rpx;margin:20rpx;">
			<view class="flexJusp">
				...
				<view style="color: #F29100;">	
					...
					<text v-if="item.pay_status==3">已完成</text>
			...
			<view class="flexJusp" style="padding-top:24rpx;border-top:2rpx solid #F7F7F7;align-items: center;margin-top:24rpx;">
				...
				<view style="font-size:24rpx;color:#666;">总价:<text style="font-weight: 800;font-size: 30rpx;color: #EC0000;" v-if="item.type==1">￥{{item.orderinfo.pickup_driver_fee}}</text><text style="font-weight: 800;font-size: 30rpx;color: #EC0000;" v-if="item.type==2">￥{{item.logistics_driver_cost}}</text><text style="font-weight: 800;font-size: 30rpx;color: #EC0000;" v-if="item.type==3">￥{{item.orderinfo.shipment_driver_fee}}</text></view>
				<view class="flex">
					
					<view class="buttonx" v-if="item.status==0" @click="quxiao(item)">取消订单</view>
					<view class="buttons" v-if="item.status==0" style="margin-left:16rpx;" @click="fukuan(item)">立即接单</view>
					<view class="buttonx" v-if="item.status==1" @click="details(item)">上传图片</view>
					<view class="buttons" v-if="item.status==2" @click="details(item)" style="margin-left:16rpx;">查看详情</view>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			quxiao(item){
				this.$httpapi('Dricer/cancel', 'POST', {id:item.id}).then(res => {
					uni.showToast({
						title: res.msg,
						duration: 1500,
						icon:'none'
			...
			details(item){
				uni.navigateTo({
					url:'/pages/order/orderDetail/orderDetail?id='+item.order_id+'&type='+(item.type ?? '')
				})
			},
			qiehuans(type){
				this.types = type
