		...
		<view class="cascader-select">
			<view class="cascader-select-item" :style="columnStyle" v-for="(item,index) in columnIndex" :key="index">
				<view class="item" v-for="(o,i) in getColumn(index)" :key="i">
					<view @click="iconClick(o,i,index)">
						<ex-icon :name="getIcon(o,i)" :color="selectIconColor"></ex-icon>
					</view>
					<view class="item-node" @click="nodeClick(o,i,index)">
						<text class="node-text" :class="item===i?'active':''"
							:style="[isActiveItem(item===i)]">{{o[props.label]}}</text>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			initSelectVal() {
				...
				let arr = uni.$ex.utils.deepClone(this.column)
			...
			getIcon(item) {
				let map = {
					0: 'checkbox-null', // 全部未选
					1: 'checkbox-half-fill', // 部分选
			...
			clickDownEvent(o, i, index) {
				...
				let val = bol ? 0 : 2
				let tempData = uni.$ex.utils.deepClone(o)
