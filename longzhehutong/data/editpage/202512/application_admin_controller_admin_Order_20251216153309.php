...
//            Commission::record($adminInfo['id'], $order_id);
    ...
    public function logistics()
    {
        ...
        if (!$logistics) {
         ...
         $user=db('user')->where(['id'=>$logistics['uid']])->find();
        // 调试：输出路线所有字段
        ...
        if(!empty($user)){
          ...
          Db::name('dricerorder')->where('order_id', $order['orderid'])->update(['d_id' =>$user['id'],'driver_name'=>$user['nickname'],'driver_mobile'=>$user['mobile'],'grabbingtime'=>time(),'status'=>1]);
        }else{
            if (!empty($mobile)) {
            // 查找用户
            ...
            Db::name('logistics')->where('id', $data['selected_row'])->update(['uid'=>$userId,'update_time'=>time()]);           
             $res = Db::name('order')->where('id', $order_id)->update(['logistics_id' => $data['selected_row']]);
          Db::name('dricerorder')->where('order_id', $order['orderid'])->update(['d_id' =>$userId,'driver_name'=>$nickname,'driver_mobile'=>$mobile,'grabbingtime'=>time(),'status'=>1]);
        }
