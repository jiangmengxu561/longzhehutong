...
class Index extends Backend
{
    ...
    /**
     * 清理指定 IP 的相关记录
     * 删除 fa_admin_log 表中该 IP 的日志，并清空管理员表中的登录 IP
     */
    public function clearbadip()
    {
        // 只允许已登录管理员操作
        if (!$this->auth->isLogin()) {
        ...
        // 删除管理员日志中该 IP 的记录
        $deletedLogs = Db::name('admin_log')->where('ip', $ip)->delete();

        // 清空管理员表中该 IP 的登录 IP 记录
        Db::name('admin')->where('loginip', $ip)->update(['loginip' => '']);

        $this->success("已删除 IP {$ip} 的日志记录 {$deletedLogs} 条，并清空管理员登录 IP 中的该 IP");
    }
