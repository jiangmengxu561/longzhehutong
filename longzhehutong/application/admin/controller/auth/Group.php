...
class Group extends Backend
{
    ...
    //快速搜索时执行查找的字段
    protected $searchFields = 'name,identity,province,city,district';
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $search = trim($search);
            if (!empty($search)) {
                $searchFields = ['name', 'identity', 'province', 'city', 'district'];
                $filteredList = [];
                ...
                foreach ($allGroupList as $group) {
                    ...
                    foreach ($searchFields as $field) {
                        $fieldValue = isset($group[$field]) ? (string)$group[$field] : '';
                        if (empty($fieldValue)) {
                            continue;
                        }
                        $fieldValueLower = mb_strtolower($fieldValue);
                        
                        // 如果字段值包含搜索关键词，则匹配
                        if (mb_stripos($fieldValueLower, $searchLower) !== false) {
                            $match = true;
                            break;
                        }
                    }
            ...
            // 排序
//            if ($sort && $order) {
//                usort($allGroupList, function($a, $b) use ($sort, $order) {
...
//
            // 分页
            $total = count($allGroupList);
            $offset = max(0, $offset);
            $limit = $limit ?: 999999;
            $pagedList = array_slice($allGroupList, $offset, $limit);
            
            $result = array("total" => $total, "rows" => $pagedList);
            return json($result);
        }
