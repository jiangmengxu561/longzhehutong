...
class Admin extends Backend
{
    ...
    public function _initialize()
    {
        ...
        // 获取城市列表数据（level=2表示市）
        $cityList = Db::name('area')->where('level', 2)->field('id,name')->order('id asc')->select();
        $citydata = [];
        foreach ($cityList as $city) {
            $citydata[$city['id']] = $city['name'];
        }
        $citydata = [0 => '全国'] + $citydata;
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    // 处理城市数据（字段名仍为 city，内容为城市ID）
                    $city = $this->request->post("city/a");
                    if ($city && is_array($city)) {
                        $city = array_values(array_filter($city, function ($item) {
                            return $item !== '' && $item !== null && $item !== '0' && $item !== 0;
                        }));
                        $params['city'] = $city ? json_encode($city) : '';
                    } else {
                        $params['city'] = '';
                    }
    ...
    public function edit($ids = null)
    {
        ...
        // 处理城市数据（存储在 city 字段，内容为城市ID列表）
        $cityids = [];
        $rowArray = $row->toArray();
        if (isset($rowArray['city']) && !empty($rowArray['city'])) {
            $cityData = json_decode($rowArray['city'], true);
        ...
        $this->view->assign("cityids", $cityids);
        return $this->view->fetch();
    }
