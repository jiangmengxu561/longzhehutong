...
class Backend extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $config = [
            ...
            // 确保 RequireJS 能正确解析 jquery/bootstrap，避免 404（baseUrl 为 assets/js 时的相对路径）
            'paths'          => [
        ...
        // 加盟商月度自动扣费（每日节流，仅后台管理端触发；覆盖加盟商及下辖线路/调度/财务）
        \app\admin\library\FranchiseService::ensureMonthlyFeeCharged();
    }
    ...
    protected function buildparams($searchfields = null, $relationSearch = null)
    {
        ...
        // 修复空 IN/NOT IN 导致 SQLSTATE[HY000]: General error: 2031
        $pkField = (!empty($this->model) ? $aliasName . $this->model->getPk() : 'id');
        foreach ($where as $wi => $cond) {
            if (is_array($cond) && count($cond) >= 3 && in_array(strtoupper($cond[1]), ['IN', 'NOT IN'])) {
                $list = is_array($cond[2]) ? $cond[2] : explode(',', trim((string)$cond[2]));
                $list = array_filter(array_map('trim', $list));
                if (count($list) === 0) {
                    if (strtoupper($cond[1]) === 'IN') {
                        $where[$wi] = [$pkField, '=', -1];
                    } else {
                        unset($where[$wi]);
                    }
                } else {
                    $where[$wi][2] = array_values($list);
                }
            }
        }
