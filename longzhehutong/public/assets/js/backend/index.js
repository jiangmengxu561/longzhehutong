define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            ...
            // 订单修改待审核：主框架轮询（仅超管）；最小化时靠系统通知，前台时蜂鸣+语音
            (function pendingAuditAlertMain() {
                var gid = (typeof Config !== 'undefined' && Config.admin && Config.admin.group_id) ? parseInt(Config.admin.group_id, 10) : 0;
                if (gid !== 1) return;
                var lastId = parseInt(localStorage.getItem('ordermodifylog_pending_last_id') || '0', 10) || 0;
                var unlocked = localStorage.getItem('ordermodifylog_audit_voice_unlocked') === '1';
                var audioCtx = null;
                $(document).one('click', function () {
                    unlocked = true;
                    localStorage.setItem('ordermodifylog_audit_voice_unlocked', '1');
                ...
                function check() {
                    $.ajax({
                        url: Backend.api.fixurl('ordermodifylog/pending_audit_alert'),
                        type: 'GET',
                        data: { last_id: lastId },
                        dataType: 'json',
                        ...
                        if (hasNew && msg) {
                            ...
                            if (unlocked) {
                                // 前台/最小化均尝试蜂鸣 + 语音（后台标签页部分浏览器会延后播放语音）
                                beep(3);
                                setTimeout(function () { speak(msg); }, 450);
                                notified = true;
                            }
                            if (notified && d.latest_id > 0) {
                                lastId = d.latest_id;
                                localStorage.setItem('ordermodifylog_pending_last_id', String(lastId));
                            }
