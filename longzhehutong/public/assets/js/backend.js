define(['fast', 'template', 'moment'], function (Fast, Template, Moment) {
    var Backend = {
        ...
        init: function () {
            ...
            //点击包含.btn-dialog的元素时弹出dialog（点图标/文字时从所在 a 上取 url/data，避免下拉内取错）
            $(document).on('click', '.btn-dialog,.dialogit', function (e) {
                var that = this;
                var $link = $(that).closest('a').length ? $(that).closest('a') : $(that);
                var options = $.extend({}, $link.data() || {});
                var url = Backend.api.replaceids($link[0], $link.data("url") || $link.attr('href'));
                var title = $link.attr("title") || $link.data("title") || $link.data('original-title');
                ...
                if (typeof options.confirm !== 'undefined') {
                    ...
                    window[$link.data("window") || 'self'].Backend.api.open(url, title, options);
                }
                return false;
            });
            ...
            $(document).on('click', '.btn-addtabs,.addtabsit', function (e) {
                ...
                var url = Backend.api.replaceids($link[0], $link.data("url") || $link.attr('href'));
                var title = $link.attr("title") || $link.data("title") || $link.data('original-title');
                var icon = $link.attr("icon") || $link.data("icon");
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
            ...
            $(document).on('click', '.btn-ajax,.ajaxit', function (e) {
                ...
                var $link = $(that).closest('a').length ? $(that).closest('a') : $(that);
                var options = $.extend({}, $link.data() || {});
                if (typeof options.url === 'undefined' && $link.attr("href")) {
                    options.url = $link.attr("href");
                }
                options.url = Backend.api.replaceids($link[0], options.url);
            ...
            $(document).on('click', '.btn-click,.clickit', function (e) {
                ...
                if (typeof options.tableId !== 'undefined') {
                    var index = parseInt(options.rowIndex, 10);
                ...
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
                        click.apply($link[0], [options, row, button]);
                        Layer.close(index);
                    });
                } else {
                    click.apply($link[0], [options, row, button]);
